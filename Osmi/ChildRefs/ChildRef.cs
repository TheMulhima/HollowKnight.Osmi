...
public static class ChildRef {
	...
	private static Action<Component> Compile(Type t) {
		...
		Action<Component> @delegate = dmd.Generate().CreateDelegate<Action<Component>>();
		...
		Logger.LogDebug($"[ChildRefs] Compiling {t.FullName} took {Time.realtimeSinceStartup - startTime}s");
